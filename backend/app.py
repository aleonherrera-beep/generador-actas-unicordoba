...
app = FastAPI(title="Generador de Actas Unicórdoba", version="0.1.0")
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
def llm_generate(messages: List[Dict[str, str]], max_new_tokens: int = 1800) -> str:
    tok, model = load_llm()
...
def summarize_transcript(transcript: str, meeting: Dict[str, Any]) -> str:
    ...
    chunks = split_text(transcript, 8500)
    notes = []
...
@app.get("/health")
def health():
    return {"ok": True, "service": "Generador de Actas Unicórdoba"}
...
async def api_draft(payload: Dict[str, Any]):
    ...
    return draft_minutes(transcript, meeting, attendance)
